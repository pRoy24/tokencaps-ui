...
import {getCoinDetail, getCoinDetailSuccess, getCoinDetailFailure, getCoinDailyHistoryData,
  ...
  getCoinWeeklyHistoryDataFailure, resetCoinSnaphsot, getCoinYearHistoryData, getCoinYearHistoryDataSuccess,
  getCoinYearHistoryDataFailure} from '../../actions/coins';
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    getCoinMonthHistoryData: (coin_name) => {
      dispatch(getCoinYearHistoryData(coin_name)).then((response) => {
        if (response.payload.status === 200) {
          dispatch(getCoinYearHistoryDataSuccess(response.payload.data));
        } else {
          dispatch(getCoinYearHistoryDataFailure(response.payload.data));
        }
      });
    },
    resetCoinSnaphsot: () => {
...
export default connect(mapStateToProps, mapDispatchToProps)(CoinView);